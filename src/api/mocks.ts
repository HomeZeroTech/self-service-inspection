...
function getEmbedding(label: string): number[] {
    const embedding =
        LABEL_EMBEDDINGS.labels[label as keyof typeof LABEL_EMBEDDINGS.labels];
    if (!embedding) {
...
export function mockGetSession(sessionId: string): SessionResponse {
    ...
    return {
        ...
        currentStep:
            state.status === "active"
                ? buildFullStep(state.currentStepIndex)
                : null,
...
export function mockCaptureStep(
    ...
    if (currentStep.stepId !== stepId) {
        throw {
            ...
            message:
                "This step has already been completed or is not the current step.",
        };
    }
    ...
    if (state.currentStepIndex >= INSPECTION_STEPS.length) {
        ...
        return {
            ...
            nextStep: null,
            message:
                "Inspection complete! Thank you for submitting all photos.",
        };
    }
